...
pub fn run() {
    ...
    longest_arr = (3 as u32).pow(longest_arr) - 1;
    println!("{longest_arr}");
        ...
        for i in 0..=possible_op.len() {
            ...
            for (i, el) in d.sequence.iter().enumerate() {
                ...
                match op {
                    ...
                    Operation::Concat => {
                        let concat = acc.to_string() + &*el.to_string();
                        acc = concat.parse().unwrap();
                    }
                }
            }
            //println!();
...
fn random_binary(num: usize) -> Vec<Operation> {
    let mut op = vec![Operation::Sum; 13];
    let mut index = 0;
    let mut num = num;
    ...
    loop {
        ...
        match reminder {
            2 => {
                op[index] = Operation::Concat;
            }
            1 => {
                op[index] = Operation::Sum;
            }
            0 => {
                op[index] = Operation::Molt;
            }
            _ => {}
        }
